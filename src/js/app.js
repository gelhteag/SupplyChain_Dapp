...
App = {
	...
	ipfs.util.addFromURL(cors + $("#upload").val(), (err, result) => {
	  if (err) {
	    throw err
	  }
	  
	  console.log(result[0].hash);
	  global.hash = result[0].hash;
	})
	/*
	let testFile = fs.readFileSync($("#upload").val());
    ...
    handleButtonClick: async function(event) {
        ...
        switch(processId) {
            case 1:
                return await App.designItem(event);
                break;
            case 2:
    ...
    designItem: function(event) {
        event.preventDefault();
        var processId = parseInt($(event.target).data('id'));

        App.contracts.SupplyChain.deployed().then(function(instance) {
            return instance.designItem(
                App.upc, 
                ...
                App.originFactoryLongitude,
                App.productNotes,
                App.image = global.hash
                
            );
        }).then(function(result) {
            $("#ftc-item").text(result);
            console.log('designItem',result);
        }).catch(function(err) {
    ...
    fetchItemBufferTwo: function () {
        event.preventDefault();
        var processId = parseInt($(event.target).data('id'));
        console.log($("#originFactoryName").val());                
        App.contracts.SupplyChain.deployed().then(function(instance) {
